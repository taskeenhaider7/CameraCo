...
export default class Fotes extends React.Component  {
        ...
        if( (res.hasOwnProperty("success")) && (res.response[0].hasOwnProperty("_id")) ){
          if (res.success){
            ...
            this.props.navigation.navigate({key:'Conversation',routeName:'Conversation',
              params:{conversationId:_conversation._id, title:_conversation.title, photo:_conversation.conversation_image}});
          }
        }
  ...
  updateLocation(){
    navigator.geolocation.getCurrentPosition(this.success.bind(this), this.error.bind(this), this.optionsP);
    id = navigator.geolocation.watchPosition(this.success.bind(this), this.error.bind(this), this.options);
  }
  stopObserving(){
    navigator.geolocation.clearWatch(this.id);
  }
  ...
  goToCamera(){
    const resetAction = NavigationActions.reset({
              index: 0,
              actions: [NavigationActions.navigate({key:'Note', routeName: 'Note',params:
                                                    {
                                                      latitude:this.state.latitude,
                                                      longitude:this.state.longitude,
                                                    }} )],
  ...
  cancelBtn(){
    this.setState({uri:'',preview:false,showInput:false,note:'',type:'',thumbnail:'',recordingTime:0,renderEffects:false});
  }
  ...
  onGoChat(item, conversation){
    this.props.navigation.navigate({key:'Conversation',routeName:'Conversation',
            params:{conversationId:conversation.id_conversation, title:conversation.title, photo:conversation.photo, item: item}});
  }
  renderContent(){
    return(
